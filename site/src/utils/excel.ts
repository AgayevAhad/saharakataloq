...
export const inferCategoryFromName = (
  ...
  if (lower.includes('qabyuyan')) {
    const found = categories.find(
      (c) =>
        c.id === 'dishwasher' || c.id === 'qabyuyan' || c.name.toLowerCase().includes('qabyuyan')
    );
  ...
  if (lower.includes('aspirator')) {
    const found = categories.find(
      (c) => c.id === 'hood' || c.id === 'aspirator' || c.name.toLowerCase().includes('aspirator')
    );
    if (found) return { categoryId: found.id, categoryName: found.name };
  }
  if (lower.includes('kondisioner')) {
    const found = categories.find(
      (c) => c.id === 'air_conditioner' || c.name.toLowerCase().includes('kondisioner')
    );
    if (found) return { categoryId: found.id, categoryName: found.name };
  }
  if (lower.includes('mikro')) {
    const found = categories.find(
      (c) => c.id === 'microwave' || c.name.toLowerCase().includes('mikro')
    );
    if (found) return { categoryId: found.id, categoryName: found.name };
  }
  if (
    lower.includes('plite') ||
    lower.includes('piltə') ||
    lower.includes('plitə') ||
    ...
    const found = categories.find(
      (c) =>
        ...
        c.name.toLowerCase().includes('plitə') ||
  ...
  if (lower.includes('soba')) {
    const found = categories.find(
      (c) => c.id === 'oven' || c.id === 'soba' || c.name.toLowerCase().includes('soba')
    );
    if (found) return { categoryId: found.id, categoryName: found.name };
  }
  if (lower.includes('soyuducu')) {
    const found = categories.find(
      (c) =>
        c.id === 'refrigerator' || c.id === 'soyuducu' || c.name.toLowerCase().includes('soyuducu')
    );
    if (found) return { categoryId: found.id, categoryName: found.name };
  }
...
export const importProductsFromExcel = (
  ...
  try {
    ...
    for (let i = 0; i < Math.min(rows.length, 10); i++) {
      ...
      if (
        joined.includes('model kodu') ||
        joined.includes('model_kodu') ||
        joined.includes('model kod')
      ) {
        headerRowIndex = i;
      ...
      if (
        joined.includes('xüsusiyyətlər') ||
        joined.includes('xususiyyetler') ||
        (joined.includes('məhsul adı') &&
          (joined.includes('satış qiyməti') || joined.includes('satis qiymeti')))
      ) {
        headerRowIndex = i;
    ...
    const idxTitle =
      colIndex('məhsul adı') !== -1
        ? colIndex('məhsul adı')
        : colIndex('mehsul adi') !== -1
          ? colIndex('mehsul adi')
          : colIndex('ad');
    ...
    const idxSpecs =
      colIndex('xüsusiyyətlər') !== -1 ? colIndex('xüsusiyyətlər') : colIndex('xususiyyetler');
    const idxImage =
      colIndex('şəkil') !== -1
        ? colIndex('şəkil')
        : colIndex('foto') !== -1
    ...
    const idxHighlights =
      colIndex('üstünlüklər') !== -1 ? colIndex('üstünlüklər') : colIndex('ustunlukler');
    ...
    for (let rowIndex = headerRowIndex + 1; rowIndex < rows.length; rowIndex++) {
      ...
      const brandId =
        matchedBrand?.id || (brandRaw ? brandRaw.toLowerCase().replace(/[^a-z0-9]/g, '-') : 'ardo');
      ...
      const price =
        priceRaw !== undefined && priceRaw !== '' && !isNaN(Number(priceRaw))
          ? Number(priceRaw)
          : undefined;

      const oldPrice =
        oldPriceRaw !== undefined && oldPriceRaw !== '' && !isNaN(Number(oldPriceRaw))
          ? Number(oldPriceRaw)
          : undefined;
      ...
      const product: Product = {
        ...
        media: [
          ...(imageRaw
            ? [{ id: `med-main-${rowIndex}`, type: 'image' as const, url: imageRaw }]
            : []),
          ...gallery.map((g, gIdx) => ({
            id: `med-gal-${rowIndex}-${gIdx + 1}`,
            type: 'image' as const,
            url: g,
          })),
        ],
    ...
    return {
      products: [],
      errors: [
        err instanceof Error ? `Excel oxuma xətası: ${err.message}` : 'Excel faylı oxuna bilmədi.',
      ],
    };
  }
};
