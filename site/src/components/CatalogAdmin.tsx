...
export const CatalogAdmin: React.FC<Props> = ({
  ...
  const [completeness, _setCompleteness] = useState<CompletenessFilter>('all');
  ...
  const loadLogs = useCallback(
    async (cat = logsCategory, search = logsSearch) => {
      setLogsLoading(true);
      try {
      ...
      <main className="admin-main">
                  ...
                  <option value="modified">
                    ✏️ Düzəliş edilənlər (
                    {
                      catalog.products.filter((p) =>
                        isProductModified(
                          p,
                          initial.products.find((ip) => ip.id === p.id)
                        )
                      ).length
                    }
                    )
                        ...
                        >
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                              ...
                              {(() => {
                                const initialProduct = initial.products.find(
                                  (p) => p.id === product.id
                                );
                                const isModified = isProductModified(product, initialProduct);
                                return isModified ? (
        ...
        {tab === 'brands' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <BrandRegistryStudio theme={theme} csrfToken={initial.csrfToken} />
          </div>
        )}

        {/* TAB BRAND RAIL: ANIMATED BRAND STRIP STUDIO */}
        {tab === 'brand_rail' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {/* TAB 4: CATEGORIES & MULTI-LEVEL TAXONOMY */}
        {tab === 'categories' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <CategoryTreeManager
              theme={theme}
      ...
      {brands.map((brand, index) => (
        ...
        >
          <div className="form-grid">
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  update(index, {
                    name: e.target.value,
                    slug: slugify(e.target.value) || brand.slug,
                  })
                }
              />
            </label>
            ...
            <label>
              <span>Təsdiqlənmə Statusu (PIM v2)</span>
              <select
                value={(brand as any).verificationStatus || 'verified'}
                onChange={(e) =>
                  update(index, { verificationStatus: e.target.value as any } as any)
                }
              >
                <option value="verified">✅ Verified (Təsdiqlənmiş)</option>
                <option value="provisional">⏳ Provisional (Müvəqqəti)</option>
                <option value="unverified">⚠️ Unverified (Təsdiqlənməmiş)</option>
              </select>
            </label>
            <label>
              <span>Brend Ləqəbləri / Sinonimlər (vergüllə)</span>
              <input
                value={
                  Array.isArray((brand as any).aliases) ? (brand as any).aliases.join(', ') : ''
                }
                onChange={(e) =>
                  update(index, {
                ...
                placeholder="Məs: Ardo Sabaf, ARDO Italy"
              />
            </label>
          </div>
          ...
          <div className="manager-footer" style={{ marginTop: '12px' }}>
            ...
            <button
              onClick={() =>
                window.confirm('Brend silinsin?') && onChange(brands.filter((_, i) => i !== index))
              }
            >
              <Trash2 size={15} /> Sil
            </button>
  ...
  return (
    <div className="manager-list">
      ...
      {categories.map((category, index) => (
        <article
          key={category.id}
          className="manager-card compact"
          style={{ background: theme.bgCard, borderColor: theme.border }}
        >
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  update(index, {
                    name: e.target.value,
                    slug: slugify(e.target.value) || category.slug,
                  })
                }
              />
            </label>
          ...
          <div className="manager-footer" style={{ marginTop: '10px' }}>
            ...
            <button
              onClick={() =>
                window.confirm('Kateqoriya silinsin?') &&
...
export const ProductEditor = ({
  ...
  const handleMediaDrop = (targetIndex: number, e: React.DragEvent) => {
    ...
    const _targetPos = mediaDropPosition === 'before' ? targetIndex + 1 : targetIndex + 2;
    const finalPos =
