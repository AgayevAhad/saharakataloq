      ...
      media: [
        {
          id: 'm-2',
          type: 'image',
          url: '/media/products/lotus-alv420s.jpg',
          alt: 'Aspirator Lotus ALV420S',
          fitMode: 'contain',
          objectPosition: 'center',
        },
      ],
...
describe('App Product Click and Modal Integration E2E Test', () => {
  ...
  it('ProductDetailModal safely handles touch swipe gestures and clears timers on unmount without errors', async () => {
    ...
    const { lightTheme } = await import('../types/theme');

    const sampleProduct = MOCK_CATALOG.products[0];
    ...
    const stage = container.querySelector('.product-detail-image-stage');
    expect(stage).not.toBeNull();

    // Trigger touch swipe gesture
    if (stage) {
      fireEvent.touchStart(stage, { touches: [{ clientX: 200, clientY: 100 }] });
      fireEvent.touchMove(stage, { touches: [{ clientX: 100, clientY: 100 }] });
    ...
    // Advance timers past timeout - should not throw or set state on unmounted component
    expect(() => {
