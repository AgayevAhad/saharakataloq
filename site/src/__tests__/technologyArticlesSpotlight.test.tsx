...
const SAMPLE_TEST_ARTICLES = [
  {
    id: 'art-inverter',
    title: 'Məişət Texnikasında İnvertor Texnologiyası',
    subtitle: 'Maksimum Səmərəlilik & Qənaət',
    badge: '⚡ İnvertor Mühərrik',
    icon: 'Zap',
    active: true,
    advantages: [
      {
        title: 'Tənzimlənən enerji istifadəsi',
        desc: 'A+++ səviyyəsində enerjiyə 50%-dək qənaət.',
      },
      { title: 'Səs və vibrasiyanın azaldılması', desc: 'Sakit işləmə rejimi.' },
    ],
  },
  {
    id: 'art-sabaf',
    title: 'İtalyan SABAF Qaz Yanma Sistemi',
    subtitle: 'İtalyan Standartı & Təhlükəsizlik',
    badge: '🔥 İtalyan Mühəndisliyi',
    icon: 'Flame',
    active: true,
    advantages: [
      { title: 'Qaz Nəzarət Sistemi (Gas Control)', desc: 'Alov söndükdə qazı avtomatik kəsir.' },
      { title: 'Mavi alov texnologiyası ilə səmərəli istilik', desc: 'Qaz sərfiyyatını azaldır.' },
    ],
  },
  {
    id: 'art-lotus-rapidair',
    title: 'Lotus 360° Rapid Air & Sağlam Qızartma',
    subtitle: 'Yağsız & Xırçıltılı Bişirmə',
    badge: '🪷 Lotus Airfryer',
    icon: 'Wind',
    active: true,
    advantages: [{ title: '360° İntensiv İsti Hava', desc: 'Yağsız xırçıltılı yeməklər.' }],
  },
  {
    id: 'art-convection',
    title: '3D Dairəvi Konveksiya və Bərabər Bişirmə',
    subtitle: 'Hər Tərəfdən Eyni Dərəcədə İstilik',
    badge: '🌪️ 3D Konveksiya',
    icon: 'RotateCw',
    active: true,
    advantages: [
      { title: 'Bərabər İstilik Sirkulyasiyası', desc: 'Hər tərəfi eyni dərəcədə bişirir.' },
    ],
  },
];

describe('Multi-Brand Technology & Promotional Spotlight Suite', () => {
  it('DEFAULT_ARTICLES is unconfigured/empty until approved by admin', () => {
    expect(DEFAULT_ARTICLES).toEqual([]);
  });

  it('BannerHero renders the technology spotlight and smoothly cycles between multi-brand articles when provided', () => {
    const onOpenArticle = vi.fn();

    render(
      <BannerHero
        theme={lightTheme}
        articles={SAMPLE_TEST_ARTICLES}
  ...
  it('InverterInfoModal seamlessly switches between multi-brand technology tabs and updates content dynamically', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 3, name: /Məişət Texnikasında İnvertor Texnologiyası/i })
    ).toBeDefined();
    expect(screen.getByText(/Tənzimlənən enerji istifadəsi/i)).toBeDefined();

    // Click on the SABAF tab button
    const sabafTab = screen.getByText(/🔥 İtalyan Mühəndisliyi/i);
    fireEvent.click(sabafTab);

    // Verify content dynamically updated to SABAF and stayed selected
    expect(
      screen.getByRole('heading', { level: 3, name: /İtalyan SABAF Qaz Yanma Sistemi/i })
    ).toBeDefined();
    expect(screen.getByText(/Qaz Nəzarət Sistemi \(Gas Control\)/i)).toBeDefined();
    expect(screen.getByText(/Mavi alov texnologiyası ilə səmərəli istilik/i)).toBeDefined();
    ...
    expect(
      screen.getByRole('heading', { level: 3, name: /Lotus 360° Rapid Air & Sağlam Qızartma/i })
    ).toBeDefined();
    ...
    expect(
      screen.getByRole('heading', { level: 3, name: /3D Dairəvi Konveksiya və Bərabər Bişirmə/i })
    ).toBeDefined();
