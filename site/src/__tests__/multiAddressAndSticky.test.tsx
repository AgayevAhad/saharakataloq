...
describe('Çoxsaylı Ünvanlar (Multiple Addresses) və Sticky Header Testləri', () => {
  ...
  it('Header komponenti sabit (sticky) pozisiya, top: 0 və z-index: 50 (sticky token) xüsusiyyətlərinə malikdir', () => {
    ...
    expect(header?.getAttribute('style')).toContain('z-index: 50');
  });

  it('SiteHeader komponenti site-header-wrapper daxilində entrapment olmadan sticky top: 0 xüsusiyyətini qoruyur', () => {
    const { container } = render(
      <div style={{ height: '3000px' }}>
        <SiteHeader
          ...
          settings={{
            ...
            addresses: [{ id: '1', title: 'Sədərək', address: 'Sədərək TM' }],
          }}
          theme={lightTheme}
          themeMode="light"
          onToggleTheme={() => {}}
          ...
          onOpenSaharaMatch={() => {}}
        />
        <main style={{ height: '2500px' }}>Long page content</main>
      </div>
    );

    const stickyHeader = container.querySelector('.site-header-sticky');
