...
describe('Sahara Electronic - UI Komponentləri və İstifadəçi Qarşılıqlı Əlaqə Testləri', () => {
  ...
  it('Artel bölməsini Tezliklə statusu ilə göstərməlidir', () => {
    ...
    expect(screen.getByAltText('ARTEL loqosu').getAttribute('src')).toBe(
      '/media/brands/artel-logo.svg'
    );
  });
  ...
  const SAMPLE_ARTICLES = [
    {
      ...
      subtitle: 'Maksimum Qənaət',
      badge: '⚡ İnvertor Mühərrik',
      icon: 'Zap',
      active: true,
      advantages: [
        { title: 'Tənzimlənən enerji istifadəsi', desc: 'Qənaət' },
        { title: 'Səs və vibrasiyanın azaldılması', desc: 'Sakit' },
      ],
    },
    {
      id: 'art-sabaf',
      title: 'İtalyan SABAF Qaz Yanma Sistemi',
      subtitle: 'İtalyan Standartı',
      badge: '🔥 İtalyan Mühəndisliyi',
      icon: 'Flame',
      active: true,
      advantages: [{ title: 'Qaz Nəzarət Sistemi', desc: 'Təhlükəsizlik' }],
    },
  ];

  it('BannerHero texnologiya karuselini göstərir və toxunanda açır', () => {
    const onOpen = vi.fn();
    render(<BannerHero theme={lightTheme} articles={SAMPLE_ARTICLES} onOpenArticle={onOpen} />);
  ...
  it('Header komponenti axtarış inputunu və nəticə sayğacını göstərir', () => {
    ...
    render(
      <Header
        ...
        onSelectBrand={vi.fn()}
        brands={DEFAULT_BRANDS}
        categories={[{ id: 'hood', name: 'Aspiratorlar', slug: 'aspiratorlar', active: true }]}
  ...
  it('FloatingActions yuxarı düyməsi yalnız səhifə aşağı sürüşdürüldükdə (scroll > 280) görünür', () => {
    ...
    const { container } = render(
      <FloatingActions settings={settings} theme={lightTheme} showToast={vi.fn()} />
    );
