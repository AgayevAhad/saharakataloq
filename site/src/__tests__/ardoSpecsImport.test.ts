...
Qaz nəzarəti: Var`;
  ...
  it('infers correct category from product title and spec cues', () => {
    expect(inferCategoryFromName('ARDO 60 sm İnox Aspirator', categories).categoryId).toBe(
      'aspirator'
    );
    expect(inferCategoryFromName('ARDO 4 gözlü Qaz Plitəsi', categories).categoryId).toBe('plite');
    expect(inferCategoryFromName('ARDO Elektrikli Daxili Soba 65L', categories).categoryId).toBe(
      'soba'
    );
    expect(
      inferCategoryFromName('ARDO Tam İnteqrasiya Edilən Qabyuyan', categories).categoryId
    ).toBe('qabyuyan');
    expect(inferCategoryFromName('ARDO İkikameralı Soyuducu No-Frost', categories).categoryId).toBe(
      'soyuducu'
    );
  });
